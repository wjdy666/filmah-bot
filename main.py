...
async def search_channels_for_file(query_text):
    ...
    try:
        if not await client_helper.is_user_authorized():
            logger.warning("الحساب المساعد غير مصرح به حالياً أو الجلسة منتهية.")
            return None
        ...
        for channel in MOVIE_CHANNELS:
            try:
                async for message in client_helper.iter_messages(channel, search=clean_query, limit=3):
...
async def send_movie_card(context, chat_id, movie):
    try:
        ...
        except Exception as helper_search_err:
            logger.error(f"Helper search safe-skip: {helper_search_err}")

        # إضافة أزرارك المعتمدة الأصلية بالملي
...
# دالة لتشغيل الحساب المساعد في خلفية منفصلة ومحمية تماماً منعاً للتعليق
async def run_helper_bot():
    global client_helper
    if SESSION_STRING:
        try:
...
async def startup_event():
    try:
        # تشغيل الحساب المساعد كـ Task منفصلة تماماً لضمان عدم حجز الـ Startup والتعليق
        asyncio.create_task(run_helper_bot())

        # إعداد البوت الرئيسي وتفعيله فورا
        ...
        # تشغيل الـ Polling في خلفية منفصلة متوافقة مع سيرفر ريندر
        asyncio.create_task(application.updater.start_polling(drop_pending_updates=True))
        logger.info("🔥 البوت الرئيسي وسيرفر FastAPI يعملان بكفاءة واستقرار تام!")
